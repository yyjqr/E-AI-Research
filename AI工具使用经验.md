...
Visual Reasoning: Opus 4.5 holds its own at 72.0% (MMM⁠⁠U Pro), sitting right between the GPT class and the new Gemini.
...
Opus 4.5 | 📁Daft | 🔀fix/colab-pydantic-pickle (0 files uncommitted, synced) | █░░░░░░░░░ 12% of 200k tokens used (/context) 💬 Okay, and this part I don't quite understand. What is type checking and why are we using it there? from typing impor... 
...
superwhisper
MacWhisper
Super Voice Assistant (open source, I built it with Claude Code)
...
https://preview.redd.it/os02fjv5nm5g1.png?width=1920&format=png&auto=webp&s=387d655b70bb141ab816cbc5c28947550a233c36
...
alias c='claude' alias gb='github' alias co='code' alias q='cd ~/Desktop/projects' 
...
⏺ Write(experiments/system-prompt-extraction/NEXT-STEPS.md) ⎿ Wrote 129 lines to experiments/system-prompt-extraction/NEXT-STEPS.md # System Prompt Slimming - Handoff Document ## Goal Reduce Claude Code's system prompt by ~45% (currently at 11%, need ~34% more). ## Current Progress ### What's Been Done - **Backup/restore system**: `backup-cli.sh` and `restore-cli.sh` with SHA256 verification - **Patch system**: `patch-cli.js` that restores from backup then applies patches ... 
...
This uses the same tmux pattern from Tip 8 - start a session, send commands, capture output. The skill file goes in ~/.claude/skills/reddit-fetch/SKILL.md. See the full skill file here.
